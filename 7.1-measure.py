...
try:
    ...
    print(f"Первая часть закончилась за: {highest_time}")
    ...
    print(f"Вторая часть закончилась за: {lowest_time}")
    ...
    with open(f"settings-{date}.txt", 'w') as f:
        f.write(f"Частота: {lowest_time/len(values)} s \nДискретка:{3.3/256} V\n")

    print(
        f"Время эксперимента: {lowest_time} s", 
        f"Average time of measurement: {lowest_time/2} s", 
        f"Частота: {lowest_time/len(values)} s", 
        f"Дискретизация: {3.3/256} V", 
        sep='\n'
    )
